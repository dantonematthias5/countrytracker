...
function buildFocusPanel(focus, level) {
  ...
  // A region-level map only makes sense when the focus is a country
  // (level 1) and a map for that specific country is available.
  if (level === 1) {
    appendCountryMap(panel, focus);
  }

  return panel;
}

// A handful of Italian region names differ from the English names the
// map itself uses. Everything else already shares the map's own names,
// since that's where the curtain's suggestion lists came from.
const ITALY_REGION_ALIASES = {
  "piemonte": "piedmont",
...
function regionMapNameFor(countryName, regionName) {
  const key = regionName.trim().toLowerCase();
  if (countryName.toLowerCase() === "italy" && ITALY_REGION_ALIASES[key]) {
    return ITALY_REGION_ALIASES[key];
  }
  return key;
}

function appendCountryMap(panel, focus) {
  ...
  section.innerHTML = `<p class="eyebrow">Regions visited in ${focus.name}</p><div class="country-map"></div>`;
  panel.appendChild(section);

  const mapEl = section.querySelector(".country-map");
  fetch(`maps/${match.map}.svg`)
    .then((res) => res.text())
    .then((svgText) => {
      mapEl.innerHTML = svgText;
      mapEl.querySelectorAll("path[aria-label]").forEach((path) => {
        const title = document.createElementNS("http://www.w3.org/2000/svg", "title");
        title.textContent = path.getAttribute("aria-label");
        path.appendChild(title);
      });
      const visitedNames = new Set(
        (focus.provinces || []).map((p) => regionMapNameFor(focus.name, p.name))
      );
      mapEl.querySelectorAll("path[aria-label]").forEach((path) => {
